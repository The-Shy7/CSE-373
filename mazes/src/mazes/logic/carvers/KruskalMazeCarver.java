...
public class KruskalMazeCarver extends MazeCarver {
    ...
    protected Set<Wall> chooseWallsToRemove(Set<Wall> walls) {
        ...
        for (Wall wall : walls) {
            ...
            edges.add(new EdgeWithData<>(wall.getRoom1(), wall.getRoom2(), edgeWeight, wall));
        }
        ...
        Set<Wall> removedEdges = new HashSet<>();
        Collection<EdgeWithData<Room, Wall>> mstEdges = mst.edges();

        for (EdgeWithData<Room, Wall> e : mstEdges) {
            removedEdges.add(e.data());
        }
